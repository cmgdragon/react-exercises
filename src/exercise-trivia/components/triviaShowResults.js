...
const TriviaShowResults = ({ user, results, spentTime, tag }) => {
    ...
    useEffect(() => {
        Promise.all([
            triviaService.addNewGlobalMark({ nickname: results.nickname, points: results.points, spentTime: spentTime, tag: tag }),
            triviaService.addNewUserMark(user.uid, { nickname: results.nickname, points: results.points, spentTime: spentTime, tag: tag }),
    ...
    return (
        <>
            ...
            <div className={'results'}>Tag: {tag}</div>
